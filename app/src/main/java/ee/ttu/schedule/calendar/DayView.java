...
public class DayView extends View implements ScaleGestureDetector.OnScaleGestureListener {
    ...
    private final Runnable mTZUpdater = new Runnable() {
        @Override
        public void run() {
            String tz = getTimeZone(mContext, this);
...
//                    GeneralPreferences.KEY_DEFAULT_CELL_HEIGHT, DEFAULT_CELL_HEIGHT);
    ...
    private void init(Context context) {
        ...
        mCurrentTime = new Time(getTimeZone(context, mTZUpdater));
...
//Todo
        ...
        mBaseDate = new Time(getTimeZone(context, mTZUpdater));
    ...
    public void handleOnResume() {
        //TODO
        initAccessibilityVariables();
//        if (getSharedPreference(mContext, OtherPreferences.KEY_OTHER_1, false)) {
//            mFutureBgColor = 0;
//        } else {
...
//        }
        ...
        mHourStrs = new String[]{ "00", "01", "02", "03", "04", "05",
    ...
    public void setSelected(Calendar calendar, boolean ignoreTime, boolean animateToday) {
        mBaseDate.set(calendar.getTimeInMillis());
    ...
    private void appendEventAccessibilityString(StringBuilder b, Event calEvent) {
        ...
        when = formatDateRange(mContext, calEvent.startMillis, calEvent.endMillis, flags);
        b.append(when);
        b.append(PERIOD_SPACE);
    }
    ...
    private View switchViews(boolean forward, float xOffSet, float width, float velocity) {
        ...
        final Calendar start = GregorianCalendar.getInstance(TimeZone.getTimeZone(mBaseDate.timezone));
...
//        }
        ...
        Time weekStart = new Time(getTimeZone(mContext, mTZUpdater));
    ...
    private void drawDayHeaderLoop(Rect r, Canvas canvas, Paint p) {
        ...
        for (int day = 0; day < mNumDays; day++, cell++) {
            ...
            if (mNumDays == 1) {
                ...
                if (isSaturday(column, mFirstDayOfWeek)) {
                    color = mWeek_saturdayColor;
                } else if (isSunday(column, mFirstDayOfWeek)) {
                    color = mWeek_sundayColor;
                }
            }
            p.setColor(color);
...
//            }
    ...
    private Rect drawEventRect(Event event, Canvas canvas, Paint p, Paint eventTextPaint,
        ...
        switch (event.selfAttendeeStatus) {
            ...
            case Attendees.ATTENDEE_STATUS_DECLINED:
                if (event != mClickedEvent) {
                    color = getDeclinedColorFromColor(color);
                }
            case Attendees.ATTENDEE_STATUS_NONE: // Your own events
            case Attendees.ATTENDEE_STATUS_ACCEPTED:
            case Attendees.ATTENDEE_STATUS_TENTATIVE:
